...
| chem_scheme_markers = | markers denoting various sections of the user's chemical scheme.  If left empty defaults to Kinetic Pre-Processor (KPP) formatting.  If filled, must have following elements separated with commas: marker for punctuation at start of gas-phase reaction lines (just the first element), marker for peroxy radical list starting, punctuation between peroxy radical names, prefix to peroxy radical name, string after peroxy radical name, number of lines taken by peroxy radical list (including the line containing the marker for peroxy radical list starting, marker at the end of each line containing generic rate coefficients, marker for start of aqueous-phase reaction lines (just the first element), marker for end of reaction rate coefficient section (also marks start of chemical reaction section) of an equation line, marker for end of equation lines (should be constant regardless of phase for equation.  For example, for the MCM FACSIMILE format: chem_scheme_markers = %, RO2, +, , , 20, ;, , :, ; |
