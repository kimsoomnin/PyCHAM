...
cwd = os.getcwd()
# 60 s intervals
(num_sb_dict['num_sb0'], num_speci_dict['num_speci0'], Cfac_dict['Cfac0'], y_dict['y0'], N_dict['N0'], sbb_dict['sbb0'], x_dict['x0'], thr_dict['thr0']) = retr(str(cwd + '/photo_chem_data/PyCHAM_time_res/PyCHAM_time_res60s'))
# 600 s intervals
(num_sb_dict['num_sb1'], num_speci_dict['num_speci1'], Cfac_dict['Cfac1'], y_dict['y1'], N_dict['N1'], sbb_dict['sbb1'], x_dict['x1'], thr_dict['thr1']) = retr(str(cwd + '/photo_chem_data/PyCHAM_time_res/PyCHAM_time_res600s'))
# 6000 s intervals
(num_sb_dict['num_sb2'], num_speci_dict['num_speci2'], Cfac_dict['Cfac2'], y_dict['y2'], N_dict['N2'], sbb_dict['sbb2'], x_dict['x2'], thr_dict['thr2']) = retr(str(cwd + '/photo_chem_data/PyCHAM_time_res/PyCHAM_time_res6000s'))
...
# O3
ax0.plot(thr_dict['thr1'], (y_dict['y1'][:,1]-y_dict['y0'][0::10,1])/np.max(np.abs(y_dict['y0'][0::10,1]))*100.0, '-r', label=r'$\mathrm{O_3}$ - $\mathrm{6x10^{2}\, s}$')
ax0.plot(thr_dict['thr2'][0:-1], (y_dict['y2'][0:-1,1]-y_dict['y0'][0::100,1])/np.max(np.abs(y_dict['y0'][0::100,1]))*100.0, '--r', label=r'$\mathrm{O_3}$ - $\mathrm{6x10^{3}\, s}$')
# OH
